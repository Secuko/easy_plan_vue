...
import { deleteSectionColumn, deleteCards, moveCard } from '../../api.js';
...
export default {
    ...
    data() {
        return {
            ...
            columnsSelectedCardsDictionary: null,
            columnMenuActions: [
                "Create task",
                "Select few",
                "Delete column",
    ...
    created() {
        ...
        this.columnsSelectedCardsDictionary = []
        this.columnsData.forEach((column) => {
            this.columnsSelectedCardsDictionary.push([])
        });
    },
    mounted() {
    ...
    methods: {
        ...
        onDrop(columnIndex) {
            if (this.draggedCard !== null && this.draggedFromColumn !== null) {
                const fromColumn = this.columnsData[this.draggedFromColumn].cards;
                const fromColumnId = this.columnsData[this.draggedFromColumn].id;
                const toColumn = this.columnsData[columnIndex].cards;
                const toColumnId = this.columnsData[columnIndex].id;
                ...
                //не знаю как фиксить, при 500 статусе непонятная ошибка
                moveCard(this.deskId, this.sectionId, moveData)
                    .then((status) => {
                        if (status === 200) {
                            this.prioritySorting(columnIndex);
                        } else {
                            fromColumn.push(this.draggedCard);
                            toColumn.splice(toColumn.length - 1, 1);
                            this.prioritySorting(this.draggedFromColumn);
                        }
                    })
                    .catch((error) => {
        ...
        changeColumnMenuStatus(index) {
            if (this.menuOpenStatuses[index]) {
                document.removeEventListener("click", this.handleClickOutside);
            } else {
                document.addEventListener("click", this.handleClickOutside);
            }
        ...
        //метод для закрытия выпадающих меню колонок, при клике снаружи
        handleClickOutside(event) {
            const menuContainer = this.$el.querySelector(".active-menu");
            if (menuContainer && !menuContainer.contains(event.target)) {
                for (let index = 0; index < this.menuOpenStatuses.length; index++) {
                    this.menuOpenStatuses[index] = false;
        ...
        cardActionCall(columnIndex, actionIndex) {
            switch (actionIndex) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    this.deleteSelectedCards(columnIndex)
                    this.changeColumnMenuStatus(columnIndex)
                    break;
                default:
                    break;
        ...
        async deleteColumn(columnIndex) {
            if ((await deleteSectionColumn(this.deskId, this.sectionId, this.columnsData[columnIndex].id)) == 204) {
        ...
        async deleteSelectedCards(columnIndex) {
            const arrayOfIdToDelete = this.columnsSelectedCardsDictionary[columnIndex]

            if (arrayOfIdToDelete.length != 0) {
                const columnId = this.columnsData[columnIndex].id
                if ((await deleteCards(this.deskId, this.sectionId, columnId, arrayOfIdToDelete)) == 204) {
                    const cards = this.columnsData[columnIndex].cards

                    // Новый массив без карточек с указанными id
                    const updatedCards = cards.filter(card => !arrayOfIdToDelete.includes(card.id));
                    this.columnsData[columnIndex].cards = updatedCards
                    this.columnsSelectedCardsDictionary[columnIndex] = []
                }
            }
        },
        lockDragling() {
        ...
        saveCardCheckboxState(isChecked, columnIndex, cardId) {
            console.log(isChecked)
            console.log(cardId)
            let array = this.columnsSelectedCardsDictionary[columnIndex];
            console.log(array.length)
            //добавляем или удаляем элементы из массива который хранит состояния элементов всех колонок
            if (isChecked) {
                array.push(cardId)
            } else {
                while (array.indexOf(cardId) !== -1) {
                    array.splice(array.indexOf(cardId), 1);
                }
            }
            this.columnsSelectedCardsDictionary[columnIndex] = array
        }
    },
}
</script>
...
<template>
    <div class="task-board">
        <div v-for="(column, columnIndex) in columnsData" :key="columnIndex" class="column"
            @dragover.prevent="onDragOver" @dragleave="onDragLeave" @dragenter="onDragEnter(columnIndex)"
            @drop="onDrop(columnIndex)" :class="{ hovered: isHovered === columnIndex }">
            <div class="header-container">
                <div class="header-text" @dblclick="editColumnName(columnIndex)">
                    <input v-if="columnIsEdited(columnIndex)"
                        @keyup.enter="confirmColumnTitle(columnIndex, $event.target.value)"
                        @blur="confirmColumnTitle(columnIndex, $event.target.value)" autofocus />
                    <h2 v-else>{{ column.title }}</h2>
                </div>
                <div :class="['menu-container', { 'active-menu': menuOpenStatuses[columnIndex] }]">
                    <div class="menu-button" @click="changeColumnMenuStatus(columnIndex)">
                        ⋮
                    </div>
                    <div class="dropdown-menu" v-if="checkColumnMenuStatus(columnIndex)">
                        <div class="dropdown-menu-elems-wrapper">
                            <div class="dropdown-menu-elem" v-for="(action, actionIndex) in columnMenuActions"
                                @click="cardActionCall(columnIndex, actionIndex)" :key="actionIndex">
            ...
            <div v-for="(card, cardIndex) in column.cards" :key="card.id" ref="card" class="card" draggable="true"
                @dragstart="onDragStart(card, columnIndex, cardIndex)" @dragend="onDragEnd"
                ...
                <div class="card-content-wrapper">
                    ...
                    <div class="card-checkbox">
                        <label class="custom-checkbox" v-if="checkColumnCheckboxStatus(columnIndex)">
                            <input type="checkbox"
                                @change="saveCardCheckboxState($event.target.checked, columnIndex, card.id)">
        ...
        <!-- Модальное окно -->
        <CardVue v-if="isCardVueOpen" @closed="changeModalCardVueStatus()" :cardData="this.selectedCard"
            :assigneeList="this.assigneeList" />
        <CardCreatingVue v-if="isCardCreatatingVueOpen" @cardCreated="updateCards" :deskId="this.deskId"
            :sectionId="this.sectionId" :columnId="columnsData[currentActionColumn].id"
            :columnIndex="this.currentActionColumn" :assigneeList="this.assigneeList"
