    ...
    methods: {
        ...
        async createCard() {
            ...
            if (this.checkOnCorrection()) {
                ...
                const data = await addCard(this.deskId, this.sectionId, this.columnId, cardObject)
                console.log(`Added new card ${data.id}`)

                this.$emit('cardCreated', data, this.columnIndex);
                this.closeModal()
            } else {
